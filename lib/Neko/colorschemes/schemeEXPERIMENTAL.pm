...
package Neko::colorschemes::schemeEXPERIMENTAL;
...
@EXPORT = qw( 
               EXP_buildHeadline
               EXP_buildCopyright
               EXP_buildFarblegende
               EXP_scheme
            );
               # EXP_calculateColor internal use

my $d = 0; #Debuginf 0|1

# EXP_buildCopyright( 'text' => <copyrightText> );
sub EXP_buildCopyright 
{
...
# calculate color for EXP scheme
sub EXP_calculateColor
{
    # calculate color for EXP scheme
    my %args = @_;
...
sub EXP_scheme
{
    ...
    # ttf-Font von System verwenden
    my $font_file = $Config->{'HeatMap'}{'font_file'};
    my $font = Imager::Font->new( file => $font_file ) or $self->setError( "Fehler beim Font laden im EXP_scheme: $!" );
